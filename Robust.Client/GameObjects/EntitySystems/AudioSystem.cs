...
#pragma warning restore 649
        ...
        public override void FrameUpdate(float frameTime)
        {
            ...
            foreach (var stream in PlayingClydeStreams)
            {
                ...
                {
                    stream.Source.SetPosition(stream.TrackingCoordinates.Value.ToWorld(_mapManager, EntityManager).Position);
                }
        ...
        {
            var source = _clyde.CreateAudioSource(stream);
            source.SetPosition(coordinates.ToWorld(_mapManager, EntityManager).Position);
            ApplyAudioParams(audioParams, source);
