...
namespace SS14.Shared
{
    public class QuadTree<T> where T : class, IQuadObject
    {
        ...
        public void Insert(T quadObject)
        {
            lock (syncLock)
            {
                ...
                if (root == null)
                {
                    ...
                    root = new QuadNode(new RectF(rootOrigin.ToFloat(), rootSizeF));
                }
        ...
        private void Query(RectF bounds, QuadNode node, List<T> results)
        {
            lock (syncLock)
            {
                if (node == null) return;
        ...
        private void ExpandRoot(RectF newChildBounds)
        {
            lock (syncLock)
            {
                ...
                RectF newRootBounds = new RectF((float)newX, (float)newY, (float)root.Bounds.Width * 2f, (float)root.Bounds.Height * 2f);
        ...
        public class QuadNode
        {
            ...
            public RectF Bounds { get; internal set; }
