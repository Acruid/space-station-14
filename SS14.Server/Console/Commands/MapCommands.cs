...
namespace SS14.Server.Console.Commands
{
    ...
    class LocationCommand : IClientCommand
    {
        ...
        public void Execute(IConsoleShell shell, IPlayerSession player, string[] args)
        {
            ...
            shell.SendText(player, $"MapID:{IoCManager.Resolve<IMapManager>().GetGrid(pos.GridId).ParentMap.Index} GridID:{pos.GridId} X:{pos.Position.X:N2} Y:{pos.Position.Y:N2}");
        }
    }
